...
AdventureWidgetsGridviewMixin = {}
function AdventureWidgetsGridviewMixin:OnLoad()
    self.data = {}
    self.frames = {}
    self.itemMinWidth = 0
    self.itemMaxWidth = 0
...
function AdventureWidgetsGridviewMixin:SetMinMaxSize(min, max)
    self.itemMinWidth = min;
    self.itemMaxWidth = max;
end

function AdventureWidgetsGridviewMixin:SetFixedColumnCount(count)
...
function AdventureWidgetsGridviewMixin:RemoveFrame(frame)
    local key;
...
function AdventureWidgetsGridviewMixin:InsertTable(tbl)

end

function AdventureWidgetsGridviewMixin:Flush()
    self.data = {}
    for k, f in ipairs(self.frames) do
        if f.ResetDataBinding then
            f:ResetDataBinding()
...
function AdventureWidgetsGridviewMixin:GetItemSize()
...
function AdventureWidgetsGridviewMixin:UpdateLayout()
    self:GetItemSize()
...
function AdventureWidgetsGridviewMixin:GetFrames()
    return self.frames;
end